...
const createLead = async (req, res) => {
    const { lead_name, company_name, email, phone_number, follow_up, followup_description } = req.body;
    try {
        const results = await pool.query('INSERT INTO leads (lead_name, company_name, email, phone_number, follow_up, followup_description) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *',
            [lead_name, company_name, email, phone_number, follow_up, followup_description]);
...
const updateLead = async (req, res) => {
    ...
    try {
        const results = await pool.query(
            'UPDATE leads SET lead_name = $1, company_name = $2, email = $3, phone_number = $4, follow_up = $5, followup_description = $6 WHERE id = $7 RETURNING *',
            [lead_name, company_name, email, phone_number, follow_up, followup_description, id]
        );
...
const deleteLead = async (req, res) => {
    const id = parseInt(req.params.id);
    try {
        const results = await pool.query('DELETE FROM leads WHERE id = $1 RETURNING *', [id]);
        res.status(200).json(results.rows[0]);
    } catch (error) {
